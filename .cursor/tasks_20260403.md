...
## Station Management grid: column reordering and per-column filters

**Source:** Backlog — add column reordering on `/station-management` and per-column filters on every filterable data column, with the same UX pattern as the Station name column (menu filter, i18n placeholders, Clear filters + global search behavior).

### Problem / user story

**As an** operations or fleet admin reviewing stations in the list,  
**I want** to reorder non-frozen columns for my workflow and filter any data column the same way I filter Station name,  
**So that** I can scan, compare, and narrow the grid quickly without losing parity with existing global search, clear-filters, selection, export, and column-picker behavior.

### Functional requirements

1. **Column reordering — scope**  
   - Reordering applies only to **optional data columns** currently exposed in the grid (the set driven by the column picker: e.g. status, address, phone, city, district, company, reseller, roaming, unit code — exact keys as in `STATION_MGMT_DATA_COLUMN_OPTIONS`).  
   - **Frozen columns are fixed:** selection checkbox, station code, station name (left), and actions (right) must **not** participate in drag-reorder; their order relative to each other and to the scrollable block stays as today unless product explicitly changes layout.  
   - **Interaction with frozen columns:** Reordered optional columns move only within the **non-frozen (scrollable)** region; dragging into frozen zones is not supported.  
   - **Persistence (optional):** Default is **no persistence** (order resets on full page reload). If persistence is in scope for a follow-up, default proposal is **session-only** (e.g. sessionStorage keyed by route/user) unless stakeholders require cross-session preference.

2. **Column reordering — implementation expectation**  
   - `p-table` already has `[reorderableColumns]="true"`; header cells for reorderable columns must use the PrimeNG pattern that completes reorder (e.g. `pReorderableColumn` on eligible `<th>` / row group per library docs), without breaking sort icons, filters, or frozen alignment.

3. **Per-column filters — coverage**  
   - Every **visible, filterable data column** has a **column filter** in the header, including columns that today only have sort (or no filter): address, phone, district, company, reseller, roaming, unit code, status, and **station code** if product treats it as filterable (see open questions).  
   - **Not filterable (unless product says otherwise):** selection checkbox column; actions column.

4. **Per-column filters — parity with Station name**  
   - Use **`display="menu"`** (same as name/city today).  
   - **i18n:** each filter has a **placeholder** (and any menu labels needed) under `stationMgmt.*` keys, consistent with `filterByName` / `filterByCity`.  
   - **Clear filters:** Existing **Clear filters** control clears **column filters and global search** (including clearing the global search input per current rule).  
   - **Selection:** Changing filters or global search continues to **clear row selection** as today (`onFilter`, global filter behavior, column-picker changes).  
   - **Column picker:** Hiding a column removes its filter state from the table (consistent with existing cleanup when a column key is no longer visible).

5. **Filter type guidance (by field semantics)**  
   - **Text (`type="text"`):** name (existing), address, phone, city, district, company name, reseller name, unit code; **default** match mode aligned with name/city (e.g. contains) unless locale/data requires exact match.  
   - **Station code (`stationInfoId`):** **Default:** treat as **text** filter on string representation for parity and simplicity; **Alternative** if needed: numeric constraints — document in open questions.  
   - **Roaming (`isRoaming`):** **Boolean** filter (true/false) or **tri-state** (all / yes / no) mapped to `row.isRoaming`; displayed values must align with existing roaming yes/no labels.  
   - **Status (derived):** **Custom** filter (e.g. `filterFunction` or template) because UI shows tags from **`isDeleted` + `isActive`**: filter options should match **displayed** statuses (e.g. Active, Inactive, Deleted per i18n), not raw booleans alone, unless engineering proves equivalent with a single clear rule.

6. **Global search vs column filters**  
   - **Default:** Global search continues to apply only to **name plus visible optional text fields** as today (`globalFilterFields`); **do not** silently add status/roaming to global search unless stakeholders ask (avoids surprising WYSIWYG and empty-cell edge cases). Column filters cover status/roaming explicitly.

7. **Accessibility**  
   - Reorder affordance and filter menus remain keyboard- and screen-reader usable at least to the level of other PrimeNG table patterns on the page (no regression).

### Acceptance criteria

- [x] Optional data columns can be **drag-reordered** within the scrollable area; **frozen** columns (checkbox, station code, name, actions) **do not** move.  
- [x] After reorder, **sort**, **column filters**, **global search**, and **pagination** still behave correctly (no broken headers, no misaligned body).  
- [x] **Every** visible optional data column listed in the column picker has a **menu** column filter with **i18n placeholder** (and filter copy consistent with name/city).  
- [x] **Roaming** filter matches **boolean** semantics and lines up with **yes/no** display text.  
- [x] **Status** filter matches **tag-derived** status (including **deleted** vs **inactive** vs **active**) per row model rules.  
- [x] **Company** column filter matches **company name** (the value used when logo is shown or falls back to text).  
- [x] **Clear filters** clears **all** column filters **and** global search input + global filter state; **row selection** is cleared when filters/search change as today.  
- [x] **Column picker:** hiding a column clears that column’s filter; showing it again does not resurrect old filter state unless product explicitly requires persistence of filter state (default: no).  
- [x] **Export (CSV / HTML / Excel)** remains **WYSIWYG** relative to **visible columns and current filter/sort** (processed data); no export of hidden columns.  
- [x] **`ng build`** succeeds with no new errors.  
- [x] **Manual smoke:** reorder + filter + clear + global search + column picker + export in **LTR**; spot-check **RTL** if the app supports it for this page.

### Implementation record (2026-04-04)

- **`station-management-list.ts`:** `[columns]` scrollable defs, `pReorderableColumn`, `dataColumnOrder` + `(onColReorder)`; menu filters with **contains** on text fields; **station code** on frozen column; **`statusCategory`** on row + **`FilterMatchMode.IN`** multi-select; **`FilterService.register('stationMgmtRoamingTri')`** + tri-state **`p-select`**; column-picker clears `statusCategory` / `isRoaming` / field keys; export column order follows **`dataColumnOrder`** ∩ visible.
- **`station-management.model.ts` / `station-management.service.ts`:** `statusCategory` derived from `isDeleted` / `isActive`.
- **`translations.ts`:** `stationMgmt.filterBy*` + `stationMgmt.filter.roaming.*` for **en, tr, fr, de**.

### Edge cases and negative scenarios

| Area | Scenario | Expected / note |
|------|-----------|------------------|
| Column picker | User hides a filtered column | Filter removed; table refilters; selection cleared per existing behavior. |
| Column picker | User shows a column again | Default: no stale filter; column appears unfilters unless persistence is explicitly required. |
| Reorder + frozen | User attempts to drag frozen header | No reorder; no console errors. |
| Export | Filters active, some columns hidden | Export columns and rows match **visible** definition and **filtered** row set. |
| Status | Row is deleted vs inactive | Filter must distinguish **deleted** from **inactive** consistently with `statusLabel` / tag severity. |
| Roaming | `isRoaming` true/false | Filter options map to stored boolean, labels match i18n. |
| Company | Logo vs text | Filter targets **name** string; logo is display-only. |
| Empty values | Blank address, phone, unit code “—” display | Text filters behave predictably (empty matches or “no match” — align with PrimeNG default; document if special handling needed). |
| Station code | Numeric ID | Leading zeros / typing “12” vs “012” if treated as text — confirm default text **contains**. |
| RTL | Mirrored layout | Headers, drag handle, and filter menus remain usable; no overlapping frozen columns. |
| Performance | Large page size (e.g. 50 rows) | Reorder and filter menus remain responsive (no functional NFR number unless product adds one). |

### Out of scope

- Persisting column order to **backend** or **cross-device** user profiles (unless later story).  
- Changing **which** columns are frozen or redesigning the **actions** column.  
- Adding **global search** fields for status/roaming by default.  
- New **saved views** (named filter + column layouts).  
- Automated E2E or visual regression tests (unless backlog explicitly adds them).

### Open questions (with suggested defaults)

1. **Should `stationInfoId` (station code) get a column filter?** — **Default:** **Yes**, text **contains** on string form for parity with other columns.  
2. **Persist column order?** — **Default:** **No** persistence v1; optional **session-only** in v1.1 if users ask.  
3. **Status filter UI:** single multi-select of three statuses vs separate rules? — **Default:** **multi-select** (or equivalent) matching the three displayed states.  
4. **Roaming filter:** strict boolean vs “any” option? — **Default:** **tri-state** (all / yes / no) for clearest UX.  
5. **Match mode** for new text filters: always **contains** like name? — **Default:** **Yes**, unless a column needs **equals** (e.g. unit code) — confirm with data cardinality.

### Validation

- [x] **`ng build`** succeeds (verified 2026-04-04).  
- [x] **Manual:** Acceptance checklist executed for implementation; re-smoke after material changes to table, filters, or demo data.
